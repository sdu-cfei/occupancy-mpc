...
package MShootBS2019
  ...
  model ZoneCO2R2C2 "Simple zone"
    ...
    Modelica.Thermal.HeatTransfer.Components.ThermalResistor re1(R=RInt/(3*Vi^(2/3)))
      annotation (Placement(transformation(extent={{76,148},{96,168}})));
    Modelica.Thermal.HeatTransfer.Components.HeatCapacitor cint(C=imass*1.2*1005*
          Vi, T(fixed=false))
      annotation (Placement(transformation(extent={{108,158},{128,178}})));
  equation
    connect(solrad, solarCoeff.u) annotation (Line(points={{-228,166},{-198,166}},
    ...
    connect(MetabolicHeat.y[1], occeffectiv.u) annotation (Line(points={{-130.8,10},
            {-118,10}},                   color={0,0,127}));
    connect(temperatureSensor.T, MetabolicHeat.u[1]) annotation (Line(points={{82,120},
            {90,120},{90,-12},{-168,-12},{-168,10},{-158.4,10}},
          color={0,0,127}));
    connect(re.port_b, cair.port) annotation (Line(points={{-100,90},{-80,90},{
            -80,120},{46,120}}, color={191,0,0}));
    connect(prescribedHeatFlow.port, cair.port) annotation (Line(points={{-138,
    ...
    connect(scale1.y, heating.u)
      annotation (Line(points={{-139,-136},{-90,-136}}, color={0,0,127}));
    connect(occ, co2.persons) annotation (Line(points={{-226,34},{-80,34},{-80,
            66},{15.6,66}},
                        color={0,0,127}));
    connect(add.y, co2.Vve) annotation (Line(points={{1,80},{7.5,80},{7.5,78},{
            15.6,78}},
                  color={0,0,127}));
    connect(const1.y, add.u1) annotation (Line(points={{-33,96},{-28,96},{-28,86},
            {-22,86}}, color={0,0,127}));
    connect(ventilation.y, add.u2) annotation (Line(points={{-67,-78},{-28,-78},{-28,
            74},{-22,74}}, color={0,0,127}));
    connect(co2.CO2, CO2)
      annotation (Line(points={{36.6,72},{132,72},{132,70},{226,70}},
                                                    color={0,0,127}));
    connect(re1.port_a, cair.port) annotation (Line(points={{76,158},{58,158},{58,
            120},{46,120}}, color={191,0,0}));
    connect(re1.port_b, cint.port)
      annotation (Line(points={{96,158},{118,158}}, color={191,0,0}));
    connect(Tout, prescribedTemperature.T) annotation (Line(points={{-226,90},{
            -195,90},{-195,90},{-162,90}}, color={0,0,127}));
    connect(Tout, max1.u1) annotation (Line(points={{-226,90},{-180,90},{-180,
            -32},{-132,-32}}, color={0,0,127}));
    connect(temperatureSensor.T, T) annotation (Line(points={{82,120},{150,120},
            {150,120},{224,120}}, color={0,0,127}));
    connect(const2.y, max1.u2)
      annotation (Line(points={{-181,-44},{-132,-44}}, color={0,0,127}));
    annotation (Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-220,
              -220},{220,220}},
          initialScale=0.1)),
      experiment(
  ...
  */
  ...
  model ZoneCO2R2C2PID "Simple zone"
    import MShootBS2019;
    ...
    Modelica.Blocks.Math.Gain scale(k=0)    annotation (Placement(transformation(
          ...
          rotation=180,
          origin={-150,-78})));
    Modelica.Blocks.Interfaces.RealInput CO2stp "CO2 setpoint [ppm]" annotation (
        Placement(transformation(extent={{-242,-92},{-214,-64}}),
          iconTransformation(extent={{-234,-60},{-206,-32}})));
    Modelica.Blocks.Sources.Constant const2(k=Tve)
      annotation (Placement(transformation(extent={{-202,-54},{-182,-34}})));
    ...
    Modelica.Blocks.Interfaces.RealInput Tstp_cooling
      "Indoor temperature setpoint [degC]" annotation (Placement(transformation(
            extent={{-256,-148},{-228,-120}}), iconTransformation(extent={{-232,
              -152},{-204,-124}})));
    Modelica.Thermal.HeatTransfer.Sensors.HeatFlowSensor heatFlowSensor1
      annotation (Placement(transformation(extent={{16,-60},{36,-40}})));
    Modelica.Blocks.Interfaces.RealOutput qv "Ventilation heat load [W]"
    ...
    Modelica.Blocks.Continuous.LimPID PIDco2(
      ...
      yMin=-100,
      controllerType=Modelica.Blocks.Types.SimpleController.PI)
      annotation (Placement(transformation(extent={{-192,-88},{-172,-68}})));
    Modelica.Blocks.Continuous.LimPID PIDt(
      k=kt,
      controllerType=Modelica.Blocks.Types.SimpleController.PI,
      yMin=-100,
    ...
    Modelica.Blocks.Continuous.LimPID PIDt1(
      k=kt,
      controllerType=Modelica.Blocks.Types.SimpleController.PI,
      yMax=100,
      yMin=0)
      annotation (Placement(transformation(extent={{-208,-216},{-188,-196}})));
    Modelica.Blocks.Interfaces.RealInput Tstp_heating
      "Indoor temperature setpoint [degC]" annotation (Placement(transformation(
            extent={{-250,-222},{-222,-194}}), iconTransformation(extent={{-232,
              -152},{-204,-124}})));
    Modelica.Blocks.Math.Add add1
      annotation (Placement(transformation(extent={{-156,-204},{-136,-184}})));
  equation
    connect(solrad, solarCoeff.u) annotation (Line(points={{-228,166},{-198,166}},
                        color={0,0,127}));
    connect(solarCoeff.y, prescribedHeatFlow.Q_flow) annotation (Line(points={{-175,
    ...
    connect(const2.y, max1.u2)
      annotation (Line(points={{-181,-44},{-132,-44}}, color={0,0,127}));
    connect(CO2stp, PIDco2.u_s)
      annotation (Line(points={{-228,-78},{-194,-78}}, color={0,0,127}));
    connect(PIDco2.y, scale.u)
      annotation (Line(points={{-171,-78},{-162,-78}}, color={0,0,127}));
    connect(Tstp_cooling, PIDt.u_s)
      annotation (Line(points={{-242,-134},{-214,-134}}, color={0,0,127}));
    connect(temperatureSensor.T, PIDt.u_m) annotation (Line(points={{82,120},{
            90,120},{90,-170},{-202,-170},{-202,-146}},
                                                color={0,0,127}));
    connect(co2.CO2, PIDco2.u_m) annotation (Line(points={{36.6,70},{70,70},{70,-106},
            {-182,-106},{-182,-90}}, color={0,0,127}));
    connect(temperatureSensor.T, PIDt1.u_m) annotation (Line(points={{82,120},{
            90,120},{90,-232},{-198,-232},{-198,-218}}, color={0,0,127}));
    connect(Tstp_heating, PIDt1.u_s) annotation (Line(points={{-236,-208},{-224,
            -208},{-224,-206},{-210,-206}}, color={0,0,127}));
    connect(PIDt1.y, add1.u2) annotation (Line(points={{-187,-206},{-164,-206},
            {-164,-200},{-158,-200}}, color={0,0,127}));
    connect(PIDt.y, add1.u1) annotation (Line(points={{-191,-134},{-162,-134},{
            -162,-188},{-158,-188}}, color={0,0,127}));
    connect(add1.y, scale1.u) annotation (Line(points={{-135,-194},{-130,-194},
            {-130,-138},{-124,-138}}, color={0,0,127}));
    connect(scale1.y, heating.u) annotation (Line(points={{-101,-138},{-96,-138},
            {-96,-136},{-90,-136}}, color={0,0,127}));
    annotation (Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-220,
              -220},{220,220}},
          initialScale=0.1)),
      experiment(
